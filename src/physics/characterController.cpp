...
void CharacterController::create(flecs::entity e, glm::vec3 startPosition,
  ...
  JPH::CapsuleShapeSettings capsuleSettings(config.capsuleHeight * 0.5f,
  ...
  JPH::CharacterVirtual *character = new JPH::CharacterVirtual(
      &settings, JPH::RVec3(startPosition.x, startPosition.y, startPosition.z),
  ...
  e.set(cc);
  Log::Print("Character Controller created for entity", "CharacterController",
             LogType::LOG_SUCCESS);
}
...
void CharacterController::destroy(flecs::entity e) {
  if (e.has<Scene::CharacterControllerComponent>()) {
    ...
    Log::Print("Character Controller destroyed for entity",
               "CharacterController", LogType::LOG_SUCCESS);
  }
}
} // namespace Cthulhu::Physics