...
std::optional<ParsedScene> JsonParser::parseScene(const std::string &path) {
  ...
  if (json.error()) {
    Log::Print("FAILED TO OPEN SCENE FILE: " + path, "JsonParser",
               LogType::LOG_ERROR);
    return std::nullopt;
  }
  ...
  for (auto entityJson : doc["entities"].get_array()) {
    ...
    if (!physicsResult.error()) {
      ...
      physics.halfExtent =
          readVec3(physicsJson["half_extent"].get_array().value());
  ...
  result.directionalLight.direction =
      readVec3(dirLightJson["direction"].get_array().value());
  result.directionalLight.color =
      readVec3(dirLightJson["color"].get_array().value());
  result.directionalLight.intensity =
      static_cast<float>(dirLightJson["intensity"].get_double().value());
  ...
  for (auto lightJson : doc["point_lights"].get_array()) {
    ...
    light.intensity =
        static_cast<float>(lightJson["intensity"].get_double().value());
    light.constant =
        static_cast<float>(lightJson["constant"].get_double().value());
    light.linear = static_cast<float>(lightJson["linear"].get_double().value());
    light.quadratic =
        static_cast<float>(lightJson["quadratic"].get_double().value());
