...
namespace Cthulhu::Scene
{
    void RegisterCoreSystems(flecs::world& world, Cthulhu::Engine* engineContext)
    {
        ...
        world.observer<PhysicsComponent>("PhysicsCleanupObserver").event(flecs::OnRemove)
        .each([engineContext]([[maybe_unused]]flecs::entity e, PhysicsComponent& physics)
        {
            if (!physics.hasBody) {return;}

            engineContext->getPhysicsWorld().removeBody(physics.bodyId);
            physics.hasBody = false;
        });

        world.observer<CharacterControllerComponent>("CharacterControllerCleanupObserver").event(flecs::OnRemove)
        .each([]([[maybe_unused]]flecs::entity e, CharacterControllerComponent& controller)
        {
            delete controller.character;
            controller.character = nullptr;
        });

        world.observer<AudioSourceComponent>("AudioCleanupObserver").event(flecs::OnRemove)
