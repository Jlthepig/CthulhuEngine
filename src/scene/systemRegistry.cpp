...
void RegisterCoreSystems(flecs::world &world, Cthulhu::Engine *engineContext) {
  world.system<TransformComponent, const PhysicsComponent>("PhysicsSyncSystem")
      .each([engineContext](TransformComponent &transform,
                            const PhysicsComponent &phys) {
        if (phys.hasBody) {
          auto bodyTransform =
              engineContext->getPhysicsWorld().getBodyTransform(phys.bodyId);
  ...
  world
      .system<CharacterControllerComponent, TransformComponent>(
          "CharacterInterpolationSystem")
      .each([engineContext]([[maybe_unused]] flecs::entity e,
  ...
  world.system<TransformComponent>("TransformSystem")
      .each([](flecs::entity e, TransformComponent &transform) {
        ...
        localMatrix = glm::rotate(localMatrix, transform.rotation.x,
                                  glm::vec3(1.0f, 0.0f, 0.0f));
        localMatrix = glm::rotate(localMatrix, transform.rotation.y,
                                  glm::vec3(0.0f, 1.0f, 0.0f));
        localMatrix = glm::rotate(localMatrix, transform.rotation.z,
        ...
        transform.cachedNormalMatrix =
            glm::transpose(glm::inverse(globalMatrix));
      });

  world
      .system<WeaponComponent, const TransformComponent, const CameraComponent>(
          "WeaponSystem")
      .each([engineContext](
                [[maybe_unused]] flecs::entity e, WeaponComponent &wep,
                const TransformComponent &trans, const CameraComponent &cam) {
        ...
        if (wep.wantsToFire) {
          ...
          if (wep.timeSinceLastShot >= cooldown) {
            ...
            Physics::RaycastHitInfo hit =
                engineContext->getPhysicsWorld().raycast(origin, forward,
                                                         wep.maxRange);
  ...
  world.system<AudioSourceComponent>("AudioSystem")
      .each([engineContext]([[maybe_unused]] flecs::entity e,
        ...
        if (audio.playTrigger) {
          ...
          auto resolvedAudioPath =
              activeProject->resolveResourcePath(audio.filePath);
          ...
          audio.soundInstanceId = Core::Audio::playSound2D(
              resolvedAudioPath->string(), audio.volume, audio.loop);
  ...
  world.observer<PhysicsComponent>("PhysicsCleanupObserver")
      ...
      .each([engineContext]([[maybe_unused]] flecs::entity e,
        ...
        engineContext->getPhysicsWorld().removeBody(physics.bodyId);
        physics.hasBody = false;
      });

  world
      .observer<CharacterControllerComponent>(
          "CharacterControllerCleanupObserver")
      .event(flecs::OnRemove)
      .each([]([[maybe_unused]] flecs::entity e,
