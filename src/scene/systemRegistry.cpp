...
void RegisterCoreSystems(flecs::world &world, Cthulhu::Engine *engineContext)
{
    ...
    world.system<CharacterControllerComponent, TransformComponent>("CharacterInterpolationSystem")
        .each([engineContext]([[maybe_unused]] flecs::entity e, CharacterControllerComponent &cc,
            ...
            if (wep.wantsToFire)
            {
                ...
                if (wep.timeSinceLastShot >= cooldown)
                {
                    ...
                    Physics::RaycastHitInfo hit =
                        engineContext->getPhysicsWorld().raycast(origin, forward, wep.maxRange);
    ...
    world.observer<CharacterControllerComponent>("CharacterControllerCleanupObserver")
        .event(flecs::OnRemove)
