...
bool SceneLoader::load(const std::string &path, Scene &scene,
  ...
  auto parsed = JsonParser::parseScene(
      path); // the parsed information provided by the json parser
  if (!parsed.has_value()) {
    Log::Print("FAILED TO LOAD SCENE: " + path, "SceneLoader",
               LogType::LOG_ERROR);
    return false;
  }
  ...
  Log::Print("Loading scene: " + parsed->name, "SceneLoader",
             LogType::LOG_INFO);
  ...
  for (auto &parsedEntity : parsed->entities) {
    ...
    if (!parsedEntity.modelPath.empty()) {
      auto resolvedModelPath =
          project.resolveResourcePath(parsedEntity.modelPath);

      if (!resolvedModelPath) {
        Log::Print("FAILED TO RESOLVE MODEL RESOURCE: " +
                       parsedEntity.modelPath,
      ...
      mesh.model =
          scene.getOrLoadModel(parsedEntity.modelPath, *resolvedModelPath);
    ...
    if (parsedEntity.physics.has_value()) {
      ...
      if (p.type == "static") {
        phys.bodyId =
            physicsWorld.addStaticBox(parsedEntity.position, p.halfExtent);
        e.add<TagStatic>();
      } else if (p.type == "dynamic") {
        phys.bodyId = physicsWorld.addDynamicBox(parsedEntity.position,
                                                 p.halfExtent, p.mass);
      }
    }
