...
void Camera::addRecoil(float pitchKick, float yawSpread) {
  ...
  currentRecoil.y = glm::min(currentRecoil.y,
                             15.0f); // caps recoil max 15 degrees upwards kick
}

void Camera::updateRecoil(float deltaTime) {
  float recoveryFactor = glm::exp(
      -8.0f *
      deltaTime); // higher = snappier recovery lower = slower or heavier
                  // recovery pistol = higher sniper rifle = lower
  currentRecoil *= recoveryFactor;

  if (glm::abs(currentRecoil.x) < 0.001f &&
      glm::abs(currentRecoil.y) < 0.001f) {
...
glm::mat4 Camera::getViewMatrix() const {
  ...
  if (currentRecoil != glm::vec2(0.0f)) {
    finalFront = glm::normalize(
        glm::rotate(glm::mat4(1.0f), glm::radians(currentRecoil.y), right) *
        glm::vec4(finalFront, 0.0f));
    finalFront = glm::normalize(
        glm::rotate(glm::mat4(1.0f), glm::radians(currentRecoil.x), up) *
        glm::vec4(finalFront, 0.0f));
  }
...
void Camera::processKeyboard(double deltaTime) {
  ...
  if (Core::Input::isKeyPressed(GLFW_KEY_ESCAPE))
    glfwSetInputMode(Core::Input::getWindowHandle(), GLFW_CURSOR,
                     GLFW_CURSOR_NORMAL);
