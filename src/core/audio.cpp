...
void Audio::init() {
  ...
  if (result != MA_SUCCESS) {
    KalaHeaders::KalaLog::Log::Print("Failed to initialize Miniaudio Engine",
                                     "Audio",
...
uint32_t Audio::playSound2D(const std::string &filePath, float volume,
                            bool loop) {
  ma_sound *pSound = new ma_sound();
  ma_result result =
      ma_sound_init_from_file(&g_audioEngine, filePath.c_str(),
                              MA_SOUND_FLAG_DECODE, NULL, NULL, pSound);
  if (result != MA_SUCCESS) {
    KalaHeaders::KalaLog::Log::Print(
        "Failed to play sound: " + std::string(filePath), "Audio",
        KalaHeaders::KalaLog::LogType::LOG_ERROR);
    delete pSound;
    return 0;
  }
