...
Window *Window::createWindow(const WindowConfig &config,
  ...
  window->glfWwindow =
      glfwCreateWindow((int)config.resolution.x, (int)config.resolution.y,
                       windowTitle, NULL, NULL);

  if (!window->glfWwindow) {
    Log::Print("FAILED TO CREATE A WINDOW.", "Main", LogType::LOG_ERROR);
    glfwTerminate();
    return nullptr;
  }
  ...
  glfwSwapInterval(
      config.vSync ? 1 : 0); // Enable or disable V-Sync based on config
  glfwSetWindowUserPointer(window->glfWwindow,
                           window_ptr); // attach this instance
...
void Window::setWindowMode(WindowMode newMode) {
  ...
  case WindowMode::Windowed: {
    glfwSetWindowMonitor(glfWwindow, nullptr, cachedX, cachedY, cachedWidth,
                         cachedHeight, 0);
  ...
  case WindowMode::Borderless: {
    ...
    glfwSetWindowMonitor(glfWwindow, monitor, 0, 0, mode->width, mode->height,
                         0);
...
float Window::getWidth() { return windowSize.x; }

float Window::getHeight() { return windowSize.y; }

GLFWwindow *Window::getWindow() const { return glfWwindow; }
...
GLFWmonitor *Window::getCurrentMonitor() const {
  ...
  if (monitorCount > 0) {
    for (int i = 0; i < monitorCount; ++i) {
      ...
      if (centerX >= xPos && centerX < xPos + mode->width && centerY >= yPos &&
          centerY < yPos + mode->height) {
        return monitors[i];
      }
    }
  }
