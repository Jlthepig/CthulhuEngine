...
bool Engine::init(const std::filesystem::path &projectFilePath) {
  ...
  if (!executableDirectory) {
    Log::Print("FAILED TO DETERMINE EXECUTABLE DIRECTORY", "ENGINE",
               LogType::LOG_ERROR);
    return false;
  }
  ...
  if (!std::filesystem::is_directory(engineResourceRoot)) {
    Log::Print("ENGINE RESOURCE DIRECTORY NOT FOUND: " +
                   engineResourceRoot.string(),
               "ENGINE", LogType::LOG_ERROR);
    return false;
  }

  glm::vec2 resolution(static_cast<float>(projectConfig.windowWidth),
                       static_cast<float>(projectConfig.windowHeight));
  ...
  window = Cthulhu::Core::Window::createWindow(windowConfig,
                                               projectConfig.name.c_str());
  ...
  if (glfwWindow == NULL) {
    Log::Print("WINDOW DOES NOT CONTAIN A VALID GLFW WINDOW", "ENGINE",
               LogType::LOG_ERROR);
    shutdown();
    return false;
  } else {
  ...
  Log::Print("ENGINE INITIALIZED FOR PROJECT: " + projectConfig.name, "ENGINE",
             LogType::LOG_SUCCESS);
...
bool Engine::loadScene(std::string_view resourcePath) {
  if (!project) {
    Log::Print("CANNOT LOAD SCENE WITHOUT AN ACTIVE PROJECT", "ENGINE",
               LogType::LOG_ERROR);
    return false;
  }
  ...
  if (!Scene::SceneLoader::load(resolvedPath->string(), *newScene, physicsWorld,
                                *project)) {
    Log::Print("FAILED TO LOAD SCENE: " + std::string(resourcePath), "ENGINE",
               LogType::LOG_ERROR);
    return false;
  }

  activateScene(std::move(newScene));
  Log::Print("ACTIVE SCENE CHANGED TO: " + activeScene->getName(), "ENGINE",
             LogType::LOG_SUCCESS);
  return true;
}
...
bool Engine::createEmptyScene(const std::string &name) {
  if (!isInitialized()) {
    Log::Print("CANNOT CREATE SCENE BEFORE ENGINE INITIALIZATION", "ENGINE",
               LogType::LOG_ERROR);
    return false;
  }
...
void Engine::processFixedUpdate(float fixedDt) {
  ...
  activeScene->getWorld().each([fixedDt,
                                this]([[maybe_unused]] flecs::entity e,
    ...
    bool isOnGround = cc.character->GetGroundState() ==
    ...
    cc.character->ExtendedUpdate(
        ...
        physicsWorld.getPhysicsSystem()->GetDefaultBroadPhaseLayerFilter(
            1), // MOVING
...
void Engine::applySimStateToSystems() {
  if (!activeScene)
    return;
  bool gameActive = (simState == SimulationState::Running ||
...
void Engine::run() {
  if (state != EngineState::Initialized) {
    Log::Print(" ENGINE MUST BE INITIALISED BEFORE RUNNING", "ENGINE",
               LogType::LOG_ERROR);
    return;
  }

  if (!glfwWindow || !activeScene) {
    Log::Print(" ENGINE RUNTIME STATE IS INVALID", "ENGINE",
               LogType::LOG_ERROR);
    return;
  }
    ...
    if (fbw > 0 && fbh > 0) {
      if (activeScene) {
        activeScene->getWorld().each(
            [&](flecs::entity e, const Scene::TransformComponent &transform,
                const Scene::MeshComponent &mesh) {
              if (e.has<Scene::TagActive>() && mesh.model) {
                frameRenderables.push_back({mesh.model,
