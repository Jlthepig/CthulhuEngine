...
void Input::init(GLFWwindow *window, glm::vec2 resolution) {
  ...
  Log::Print("Input system initialized successfully", "ENGINE",
             LogType::LOG_SUCCESS);
}

GLFWwindow *Input::getWindowHandle() { return windowHandle; }

void Input::update() {
  memcpy(previousKeys, currentKeys, sizeof(currentKeys));
  memcpy(previousMouseButtons, currentMouseButtons,
...
bool Input::isKeyDown(int key) { return currentKeys[key]; }
...
void Input::mouse_callback([[maybe_unused]] GLFWwindow *window, double xposIn,
                           double yposIn) {
...
float Input::getMouseX() { return lastX; }

float Input::getMouseY() { return lastY; }
