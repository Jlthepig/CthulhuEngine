...
void Skybox::loadHDR(const std::string &path) {
  ...
  if (!data) {
    ...
    Log::Print("HDR LOADED SUCCESSFULLY: " + path, "Skybox",
               LogType::LOG_SUCCESS);
  }
...
void Skybox::load(const std::filesystem::path &engineRoot,
  ...
  std::vector<float> verts(cubeVertices,
                           cubeVertices + sizeof(cubeVertices) / sizeof(float));
  std::vector<unsigned int> indices;

  for (unsigned int i = 0; i < 36; i++)
    indices.push_back(i);

  std::vector<Mesh::vertexAttribute> attrs = {{0, 3, 0}};
  ...
  equirectShader.load(
      (engineResourceRoot / "shaders/equirect.vertex").string(),
      (engineResourceRoot / "shaders/equirect.fragment").string());
  skyboxShader.load((engineResourceRoot / "shaders/skybox.vertex").string(),
                    (engineResourceRoot / "shaders/skybox.fragment").string());
...
void Skybox::generateIrradianceMap() {
  ...
  irradianceShader.load(
      (engineResourceRoot / "shaders/irradiance.vertex").string(),
      (engineResourceRoot / "shaders/irradiance.fragment").string());
  ...
  KalaHeaders::KalaLog::Log::Print("Irradiance map generated", "Skybox",
...
void Skybox::generatePrefilterMap() {
  ...
  prefilterShader.load(
      (engineResourceRoot / "shaders/prefilter.vertex").string(),
      (engineResourceRoot / "shaders/prefilter.fragment").string());
  prefilterShader.use();
  prefilterShader.setInt("environmentMap", 0);
  prefilterShader.setMat4(
      "projection", captureProjection); // Reuse your existing captureProjection

  glActiveTexture(GL_TEXTURE0);
  glBindTexture(GL_TEXTURE_CUBE_MAP,
                cubemapTexture); // Reuse your existing HDR cubemap
  ...
  for (unsigned int mip = 0; mip < maxMipLevels; ++mip) {
    ...
    unsigned int mipWidth =
        static_cast<unsigned int>(128 * std::pow(0.5f, mip));
    ...
    for (unsigned int i = 0; i < 6; ++i) {
      prefilterShader.setMat4("view",
                              captureViews[i]); // Reuse your existing 6 views
      glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0,
  ...
  KalaHeaders::KalaLog::Log::Print("Pre-filtered Environment Map generated",
                                   "Skybox",
                                   KalaHeaders::KalaLog::LogType::LOG_SUCCESS);
}

void Skybox::draw([[maybe_unused]] const glm::mat4 &view,
