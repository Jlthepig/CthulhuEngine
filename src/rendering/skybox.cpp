...
{
static float cubeVertices[] = {-1.0f, 1.0f,  -1.0f, -1.0f, -1.0f, -1.0f, 1.0f,  -1.0f, -1.0f,
...
void Skybox::convertToCubemap()
{
    ...
    for (unsigned int i = 0; i < 6; i++)
    {
        ...
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, cubemapTexture,
                               0);
...
void Skybox::load(const std::filesystem::path &engineRoot, const std::filesystem::path &hdrPath)
{
    ...
    equirectShader.load((engineResourceRoot / "shaders/equirect.vertex").string(),
...
void Skybox::generateIrradianceMap()
{
    ...
    irradianceShader.load((engineResourceRoot / "shaders/irradiance.vertex").string(),
...
void Skybox::generatePrefilterMap()
{
    ...
    prefilterShader.load((engineResourceRoot / "shaders/prefilter.vertex").string(),
    ...
    KalaHeaders::KalaLog::Log::Print("Pre-filtered Environment Map generated", "Skybox",
