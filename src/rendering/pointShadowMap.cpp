...
void PointLightShadowMap::init(unsigned int width, unsigned int height, const std::filesystem::path &engineResourceRoot)
{
    ...
    {
        glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, 0, GL_R32F, shadowWidth, shadowHeight, 0, GL_RED, GL_FLOAT,
                     nullptr);
    }
    ...
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X, depthCubeMap, 0);
    ...
    {
        KalaHeaders::KalaLog::Log::Print("POINT SHADOW FBO NOT COMPLETE: " + std::to_string(status), "PointShadowMap",
                                         KalaHeaders::KalaLog::LogType::LOG_ERROR);
    }
    else
    {
        KalaHeaders::KalaLog::Log::Print("Point shadow FBO complete", "PointShadowMap",
...
{
    glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, depthCubeMap,
                           0);
