...
void Framebuffer::create(unsigned int width, unsigned int height) {
  ...
  this->height = height;
  if (width == 0 || height == 0) {
    KalaHeaders::KalaLog::Log::Print("Framebuffer dimensions cannot be zero!",
                                     "Framebuffer",
  ...
  if (glCheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE) {
    KalaHeaders::KalaLog::Log::Print("Framebuffer not complete!", "Framebuffer",
                                     KalaHeaders::KalaLog::LogType::LOG_ERROR);
  } else {
    KalaHeaders::KalaLog::Log::Print(
        "Framebuffer created successfully", "Framebuffer",
...
void Framebuffer::resize(unsigned int width, unsigned int height) {
  if (width == 0 || height == 0) {
    KalaHeaders::KalaLog::Log::Print(
        "Framebuffer resize dimensions cannot be zero!", "Framebuffer",
        KalaHeaders::KalaLog::LogType::LOG_ERROR);
    return;
  }
...
void Framebuffer::blitToScreen(unsigned int screenWidth,
...
GLuint Framebuffer::getColorTexture() const { return colorTexture; }
